...
Paralelismo (Nivel 1 — corridas dentro de cada evaluación):
  · Las n_corridas de run_once por trial se ejecutan en paralelo via
    ProcessPoolExecutor, eliminando el cuello de botella secuencial.
  · La varianza σ²(x) se calcula en el proceso principal tras recibir
    todos los valores → _variance_store se escribe sin race conditions.
  · Speedup esperado: ~n_corridas× (e.g. 3× con --n_corridas 3).
  · Control: --n_workers N  (0 = auto = min(n_corridas, nCPUs)).

Uso:
  python modulo7_smac_sk.py --tipo hpo_sk --n_trials 100 --n_corridas 3
  python modulo7_smac_sk.py --tipo hpo_sk --n_trials 100 --n_corridas 3 --n_workers 3
...
def graficar(
    ...
    """
    Genera figura de 2 paneles a partir de un ResultadoSMAC_SK
    (o dict equivalente desde JSON).

    Panel 1 — Convergencia:
      · Costo de cada evaluación (puntos grises)
      · Mejor incumbente acumulado (línea azul)
      · Línea horizontal al mejor costo final

    Panel 2 — Tiempo de proceso:
      · Tiempo acumulado por evaluación (área azul)
      · Tiempo incremental por evaluación (barras grises)
      · Línea vertical al tiempo total

    Retorna la ruta del PNG guardado.
    """
    # Normalizar entrada (acepta dataclass o dict)
    ...
    # ── Panel 1: Convergencia ─────────────────────────────────
    # Puntos individuales (grises, semitransparentes)
    ...
    # Curva de incumbente acumulado
    inc_validos = [(i, v) for i, v in zip(trials, incumbentes) if not np.isnan(v)]
    if inc_validos:
        xi, yi = zip(*inc_validos)
    ...
    # Zona de diseño inicial (primeros ~10% de trials = LHS/Sobol)
    ...
    # Línea vertical al tiempo total en eje acumulado
    ...
    # Leyenda combinada de ambos ejes del panel 2
    ...
    try:
        ...
        log.warning("No pudo guardar gráfico %s: %s", guardar_png, e)
...
def optimizar(
    ...
    # Tracker de tiempo y costo por evaluación (actualizado dentro de fn_obj)
    _tracker = {"t0_opt": None, "calls": []}   # calls: [{t_seg, costo}]
    ...
    historia = []
    # Enriquecer con timestamps del tracker (orden de evaluación = orden del tracker)
    ...
    # Añadir mejor_hasta_ahora (incumbente acumulado por orden de evaluación)
    ...
    # Generar gráfico automáticamente junto al JSON
...
# Los workers con método 'spawn' reimportan el módulo; sin este guard
# relanzarían el CLI recursivamente en cada proceso hijo.
if __name__ == "__main__":
    ...
    # Propagar flag --no_plot al módulo via atributo en optimizar
