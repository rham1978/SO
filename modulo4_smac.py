...
Parámetros del modelo que se optimizan (11):
  ...
  2.  horas_control_post       → fixed_post_control_hours (pac/sem control)
...
def _aplicar_config_a_cfg(config, cfg_base):
    ...
    # 2. Horas control post → pac/sem (cada paciente = 20 min)
    if "horas_control_post" in config:
        pac_sem = int(config["horas_control_post"])
        cfg.fixed_post_control_hours  = pac_sem * 20.0 / 60.0
...
def evaluar_configuracion(
    ...
    """
    Función objetivo compartida por todos los tipos de SMAC.
    Corre el modelo con la configuración dada y retorna el costo.

    Parámetros
    ----------
    config      : ConfigSpace.Configuration
    seed        : semilla de SMAC (se suma a seed_base)
    n_corridas  : corridas por evaluación (promediadas)
    seed_base   : semilla base del modelo
    objetivo    : KPI a minimizar
    pesos_kpi   : dict {kpi: peso} para objetivo compuesto
    budget      : presupuesto multi-fidelidad (n_corridas escalado si se da)

    Retorna float (menor = mejor)
    """
    _, CFG_base, run_once, _, _ = _importar_baseline()
    cfg = _aplicar_config_a_cfg(config, CFG_base)

    # Multi-fidelidad: budget controla número de corridas
    if budget is not None:
        n = max(1, int(round(budget)))
    else:
        n = n_corridas
    ...
    for r in range(n):
        try:
            ...
            if objetivo == "compuesto" and pesos_kpi:
                val = sum(
                    float(pesos_kpi.get(k, 0.0)) * float(res.get(k, 0.0))
                    for k in pesos_kpi
                )
            else:
            ...
            log.warning("Error corrida %d (seed=%d): %s", r, seed_base+seed+r, e)
            valores.append(1e9)

    costo = float(np.mean(valores))
    log.info("  Config eval → %s=%.2f (n=%d, seed=%d)", objetivo, costo, n, seed)
    return costo
...
def optimizar(
    ...
    Parámetros
    ----------
    tipo             : tipo de SMAC ('blackbox','hpo','multifidelity','algconfig','random','hyperband')
    n_trials         : evaluaciones totales del modelo
    n_corridas_eval  : corridas del modelo por evaluación (promediadas)
    seed             : semilla SMAC
    seed_base_modelo : semilla base del modelo de simulación
    objetivo         : KPI a minimizar
    pesos_kpi        : pesos para objetivo='compuesto'
    output_dir       : directorio de salida SMAC (auto si None)
    guardar_json     : archivo JSON resultado (auto si None)
    min_budget       : presupuesto mínimo (multi-fidelidad)
    max_budget       : presupuesto máximo (multi-fidelidad)

    Retorna ResultadoSMAC con incumbente y métricas
    """
    ...
    # Función objetivo (closure con parámetros fijos)
    _pesos = pesos_kpi or {}
...
def evaluar_incumbente(
    ...
    """
    Evalúa el mejor incumbente con más corridas para obtener una estimación
    estadísticamente robusta de su desempeño real.

    Útil para validar que la mejora encontrada por SMAC es real y no artefacto
    del ruido de pocas corridas durante la optimización.
    """
    _, CFG_base, run_once, _, _ = _importar_baseline()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["wl_total_end","wl_first_end","tts_first_attended_days_mean",
    ...
    # Pesos para objetivo compuesto
    # Normalizados por valor baseline para que cada componente contribuya
    # en proporción similar, centrado en tts_full_days_mean:
    #   tts_full  baseline ≈ 279.78 días  → peso 1.0/279.78 ≈ 0.00357
    #   tts_first baseline ≈ 193.85 días  → peso 0.5/193.85 ≈ 0.00258
    #   wl_total  baseline ≈ 1660         → peso 0.3/1660   ≈ 0.00018
    pesos = None
    if args.objetivo == "compuesto":
        pesos = {
            "tts_full_days_mean":            1.0 / 279.78,   # KPI principal
            "tts_first_attended_days_mean":  0.5 / 193.85,   # primera consulta
            "wl_total_end":                  0.3 / 1660.0,   # lista espera residual
        }
        log.info("Objetivo compuesto normalizado: %s", pesos)
