"""
===============================================================================
MÓDULO 9 — SK con REVI (Replicated Expected Value of Information)
===============================================================================
Extiende el Módulo 8 (SK + Replicación Adaptativa) reemplazando la política
de asignación de réplicas por REVI, que balancea dos fuentes de incertidumbre:

  σ²_sim(x)  : varianza del simulador en x  (ruido del DES)
  σ²_mod(x)  : varianza del modelo SK en x  (incertidumbre del surrogate)

Fórmula de asignación REVI:
  n*(x) = clip( sqrt( σ²_sim(x) / σ²_mod(x) ), n_min, n_max )

Interpretación:
  · Si σ²_sim >> σ²_mod: el simulador es ruidoso y el modelo ya es preciso
    → agregar réplicas reduce la incertidumbre del estimador → n alto.
  · Si σ²_sim << σ²_mod: el modelo mismo es incierto → las réplicas extra
    se pierden en la incertidumbre del surrogate → n bajo, mejor explorar.
  · Balance óptimo: cuando el ruido de estimación ≈ incertidumbre del modelo.

Diferencia clave vs Módulo 8 (AdaptiveReplicationPolicy):
  M8:  n(x) ∝ percentil(σ²_sim(x))          — solo mira ruido del simulador
  M9:  n(x) = sqrt(σ²_sim(x) / σ²_mod(x))  — balancea simulador vs modelo

Warmup idéntico al M8: primeras n_warmup evaluaciones usan n_min réplicas
para poblar _variance_store antes de activar REVI.

Indicador fijo : tts_full_days_mean
Gráficos       : convergencia + tiempo + réplicas REVI + σ²_sim y σ²_mod

Uso:
  python modulo9_sk_revi.py --tipo hpo_sk --n_trials 100
  python modulo9_sk_revi.py --tipo hpo_sk --n_trials 100 --n_workers 4
  python modulo9_sk_revi.py --tipo hpo_sk --n_trials 100 \\
      --n_min_reps 2 --n_max_reps 8 --n_warmup 10
  python modulo9_sk_revi.py --evaluar_incumbente resultado_m9.json
  python modulo9_sk_revi.py --no_revi --n_corridas 3   # modo fijo (M8 compatible)
===============================================================================
"""
...
def graficar(
    ...
    hay_extra = (revi_mode or adaptativo) and n_reps_history and sigma_history
    n_panels = 4 if hay_extra else 2
    ...
    modo = "REVI" if revi_mode else ("Adaptativo" if adaptativo else "Fijo")
    reps_str = (f"n_reps [{n_min_reps}–{n_max_reps}] {modo}"
...
def optimizar(
    ...
    n_workers:       int   = 0,
    # ── Parámetros replicación (módulo 8/9) ───────────────────
    adaptativo:      bool  = False,   # M8: AdaptiveReplicationPolicy
    revi:            bool  = True,    # M9: REVIReplicationPolicy (default ON)
    n_min_reps:      int   = 2,
    n_max_reps:      int   = 8,
    n_warmup:        int   = 10,
) -> ResultadoSMAC_SK:
    """
    Ejecuta SMAC+SK con REVI (módulo 9) o replicación adaptativa (M8).

    revi=True       → REVIReplicationPolicy (balances σ²_sim / σ²_mod)
    adaptativo=True → AdaptiveReplicationPolicy (solo σ²_sim, M8)
    ambos False     → n_corridas_eval fijo (modo M7)
    ...
    output_dir   = output_dir   or f"smac_m9_output_{tipo}"
    guardar_json = guardar_json or f"resultado_m9_{tipo}.json"

    modo_str = ("REVI" if revi else "Adaptativo" if adaptativo else "Fijo")
    log.info("="*65)
    log.info("SMAC+SK Módulo 9 — %s", TODOS_TIPOS[tipo])
    log.info("Objetivo   : %s", objetivo)
    log.info("Modo reps  : %s  [n_min=%d  n_max=%d  n_warmup=%d]",
             modo_str, n_min_reps, n_max_reps, n_warmup)
    ...
    if tipo == "hpo_sk":
        ...
        # Guardar referencia al modelo SK para inyectarlo en REVI
