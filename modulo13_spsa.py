"""
===============================================================================
MÓDULO 13 — SPSA: Simultaneous Perturbation Stochastic Approximation
===============================================================================
Implementa SPSA de Spall (1998), adaptado al simulador DES clínico.
...
Ventaja clave sobre M11-M12 (TR):
  Solo 2 evaluaciones del simulador por iteración para estimar el gradiente,
  independientemente de la dimensión d=12.
  ASTRO-DF/STRONG necesitan d+1=13 a 2d+1=25 evaluaciones por iteración.

Algoritmo:
  Para cada iteración k:
    1. Generar Δk ∈ {-1,+1}^d (Bernoulli simétrico, iid por dimensión)
    2. ck = step / (k+1)^gamma  (paso de perturbación decreciente)
    3. Evaluar: y+ = F̄(xk + ck·Δk, n_reps)
                y- = F̄(xk - ck·Δk, n_reps)
    4. Gradiente SPSA: g_k,i = (y+ - y-) / (2·ck·Δk,i)
    5. Promediar gavg gradientes consecutivos → ĝk
    6. ak = alpha / (k + A)^alpha_exp  (paso del iterado)
    7. xk+1 = clip(xk - ak·ĝk, [0,1]^12)

Parámetros del paper (SimOpt defaults):
  alpha=0.602, gamma=0.101, step=0.1, gavg=1
  n_reps=30, A=10 (estabilización inicial)

Indicador fijo : tts_full_days_mean
Gráficos       : convergencia + tiempo + paso ak + ||ĝk|| norma gradiente
Paralelismo    : ProcessPoolExecutor para n_reps réplicas por punto

Uso:
  python modulo13_spsa.py --n_iter 200
  python modulo13_spsa.py --incumbente resultado_m10_hpo_kgcp.json
  python modulo13_spsa.py --n_iter 300 --n_reps 10 --n_workers 4
===============================================================================
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# MÓDULO 13 — SPSA (Simultaneous Perturbation Stochastic Approximation)
# Spall (1998) IEEE Trans. AES 34(3):817-823
# ══════════════════════════════════════════════════════════════════════════════
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Módulo 13: SPSA — Simultaneous Perturbation Stochastic Approximation"
    )
