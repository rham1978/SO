"""
===============================================================================
MÓDULO 14 — ALOE: Adaptive Line-search with Oracle Estimations
===============================================================================
Implementa ALOE de Jin, Scheinberg & Xie (2021), adaptado al simulador DES.

Referencia:
  Jin, B., Scheinberg, K., Xie, M. (2021). "High Probability Complexity
  Bounds for Line Search Based on Stochastic Oracles." Advances in Neural
  Information Processing Systems (NeurIPS), 34:9193-9203.

Diferencia vs SPSA (M13):
  SPSA:  gradiente por perturbación simultánea (2 eval, sesgado con ruido alto)
  ALOE:  gradiente por diferencias finitas centrales (2d eval, no sesgado) +
         condición de Armijo RELAJADA que absorbe el ruido del simulador.

Algoritmo (Jin et al. 2021):
  Para cada iteración k:
    1. Estimar gradiente gk por diferencias finitas centrales:
       g_k,i = (F̄(xk+h·ei,r) - F̄(xk-h·ei,r)) / (2h)
    2. Proponer: xk+ = clip(xk - αk·gk, [0,1]^12)
    3. Evaluar F̄(xk+, r) y F̄(xk, r) con r réplicas
    4. Condición Armijo RELAJADA (clave de ALOE):
       F̄(xk+) ≤ F̄(xk) - αk·θ·‖gk‖² + 2·εf
       El término 2·εf absorbe el ruido del simulador sin rechazar pasos buenos.
    5. Si cumple: aceptar xk+, αk+1 = min(αmax, αk/γ)  (expandir paso)
       Si no:     rechazar,   αk+1 = γ·αk               (reducir paso)

Garantías: alta probabilidad O(k^{-1/2}) para encontrar punto ε-estacionario.
           (Teorema 1 de Jin et al. 2021)

Parámetros del paper (SimOpt defaults):
  r=30 (réplicas), theta=0.2, gamma=0.8
  alpha_max=10.0, alpha_0=1.0, epsilon_f=1.0, h=0.01

Indicador fijo : tts_full_days_mean
Gráficos       : convergencia + tiempo + paso αk + εf estimado
Paralelismo    : ProcessPoolExecutor para réplicas (r) y diferencias finitas

Uso:
  python modulo14_aloe.py --n_iter 200
  python modulo14_aloe.py --incumbente resultado_m10_hpo_kgcp.json
  python modulo14_aloe.py --n_iter 200 --r 10 --n_workers 4 --h 0.02
===============================================================================
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# MÓDULO 14 — ALOE (Adaptive Line-search with Oracle Estimations)
# Jin, Scheinberg & Xie (2021) NeurIPS 34:9193-9203
# ══════════════════════════════════════════════════════════════════════════════
...
class ResultadoALOE:
    ...
    max_iter:          int   = 200
    historia_costos:   list  = field(default_factory=list)
    # {iter, costo, f_incumbente, mejor_hasta_ahora, alpha_k, eps_f_est, grad_norm,
    #  armijo_ok, t_seg}
...
def optimizar_aloe(
    ...
    epsilon_f: float = 1.0,
    h:         float = 0.01,
    guardar_json: str = "resultado_m14_aloe.json",
) -> ResultadoALOE:
    """
    ALOE: line-search con condición Armijo relajada.
    Condición: F̄(xk+) ≤ F̄(xk) - αk·θ·‖gk‖² + 2·εf
    El término 2·εf absorbe el ruido del simulador.
    """
    log.info("="*65)
    log.info("ALOE — Módulo 14  |  %s", objetivo)
    log.info("r=%d  θ=%.2f  γ=%.2f  α0=%.2f  αmax=%.2f  εf=%.3f  h=%.4f  max=%d",
             r, theta, gamma, alpha_0, alpha_max, epsilon_f, h, max_iter)
    log.info("="*65)
    ...
    historia.append({"iter": 0, "costo": round(fk, 4),
                     ...
                     "alpha_k": round(alpha, 6), "eps_f_est": round(epsilon_f, 4),
                     "grad_norm": 0.0, "armijo_ok": True, "t_seg": 0.0})
    log.info("x0: %s=%.4f", objetivo, fk)
    ...
    for k in range(1, max_iter + 1):
        ...
        # εf estimado a partir de la varianza del simulador
        eps_f_est = epsilon_f + (sk_est ** 2) / max(r, 1)

        # ── Condición Armijo relajada (ec. central de ALOE) ──────────────
        armijo_rhs = fk_est - alpha * theta * (grad_norm ** 2) + 2.0 * eps_f_est
        armijo_ok  = fkp <= armijo_rhs

        if armijo_ok:
            xk    = xk_plus.copy()
            fk    = fkp
            alpha = min(alpha_max, alpha / gamma)   # expandir paso
            if fk < f_inc:
                f_inc, x_inc = fk, xk.copy()
            log.info("k=%3d ✓  %s=%.3f  α=%.5f  ‖g‖=%.4f", k, objetivo, fk, alpha, grad_norm)
        else:
            alpha = gamma * alpha                   # reducir paso
            log.debug("k=%3d ✗  %s=%.3f  α=%.5f  ‖g‖=%.4f", k, objetivo, fkp, alpha, grad_norm)
        ...
        historia.append({
            ...
            "alpha_k": round(float(alpha), 6),
            "eps_f_est": round(float(eps_f_est), 4),
    ...
    log.info("─"*65)
    log.info("ALOE completado: %.0fs | Mejor %s = %.4f", t_total, objetivo, f_inc)

    resultado = ResultadoALOE(
        objetivo=objetivo, n_iter=len(historia), incumbente=inc_dict,
        costo_incumbente=round(f_inc, 4), tiempo_seg=round(t_total, 2),
        seed=seed, seed_base=seed_base, r=r, theta=theta, gamma=gamma,
        alpha_max=alpha_max, alpha_0=alpha_0, epsilon_f=epsilon_f,
        h=h, max_iter=max_iter, historia_costos=historia,
    )
    out = dataclasses.asdict(resultado)
    with open(guardar_json, "w", encoding="utf-8") as f:
        json.dump(out, f, ensure_ascii=False, indent=2)
    log.info("Guardado: %s", guardar_json)
    if not getattr(optimizar_aloe, "_no_plot", False):
        graficar_aloe(resultado, guardar_png=guardar_json.replace(".json", ".png"))
    return resultado


def graficar_aloe(resultado, guardar_png=None, mostrar=False):
    """4 paneles: convergencia + tiempo + paso αk + εf estimado."""
    ...
    alphas   = [h.get("alpha_k", 0.0)     for h in historia]
    epsilons = [h.get("eps_f_est", 0.0)   for h in historia]
    gnorms   = [h.get("grad_norm", 0.0)   for h in historia]
    armijos  = [h.get("armijo_ok", True)  for h in historia]
    ...
    fig.suptitle(f"ALOE — Adaptive Line-search con Oracle Estimations\n"
                 f"Objetivo: {objetivo}  ·  {n_iter} iteraciones  ·  {t_total:.0f}s totales",
                 fontsize=11, fontweight="bold", y=0.99)

    # Panel 1: Convergencia
    col_pts = ["#27ae60" if armijos[i] else "#e74c3c" for i in range(len(iters))]
    ax1.scatter(iters, costos, s=20, c=col_pts, alpha=0.65, zorder=2,
                label="Evaluaciones (verde=Armijo OK)")
    ...
    ax1.set_title("Convergencia ALOE", fontsize=10, pad=4)
    ...
    ax2.axhline(t_med, color="#e74c3c", linewidth=1.0, linestyle="--", alpha=0.7, label=f"Prom={t_med:.1f}s")
    ...
    # Panel 3: Paso αk (adaptativo)
    ax3.semilogy(iters, [max(a, 1e-12) for a in alphas],
                 color="#8e44ad", linewidth=1.8, label="αk (paso line-search)")
    ax3.axhline(r.get("alpha_max", 10.0), color="#27ae60", linewidth=0.8,
                linestyle="--", alpha=0.6, label=f"αmax={r.get('alpha_max',10.0)}")
    ax3.axhline(1e-10, color="#c0392b", linewidth=0.8, linestyle=":", alpha=0.6, label="conv tol")
    ax3.set_title("Paso adaptativo αk (log) — sube si Armijo OK, baja si no", fontsize=10, pad=4)
    ax3.set_xlabel("Iteración", fontsize=10); ax3.set_ylabel("αk", fontsize=9)
    ax3.xaxis.set_major_locator(mticker.MaxNLocator(integer=True, nbins=12))
    ax3.legend(fontsize=8)

    # Panel 4: εf estimado (ruido del simulador) y ‖gk‖
    ax4.plot(iters, epsilons, color="#c0392b", linewidth=1.5, alpha=0.8, label="εf estimado")
    ax4.fill_between(iters, epsilons, alpha=0.12, color="#c0392b")
    ax4b = ax4.twinx()
    ax4b.plot(iters, gnorms, color="#e67e22", linewidth=1.5, linestyle="--",
              alpha=0.8, label="‖gk‖ gradiente")
    ax4b.set_ylabel("‖gk‖", fontsize=9, color="#e67e22")
    ax4b.tick_params(axis="y", labelcolor="#e67e22")
    ax4.set_title("Ruido estimado εf y norma gradiente ‖gk‖", fontsize=10, pad=4)
    ax4.set_xlabel("Iteración", fontsize=10); ax4.set_ylabel("εf estimado", fontsize=9, color="#c0392b")
    ax4.tick_params(axis="y", labelcolor="#c0392b")
    ax4.xaxis.set_major_locator(mticker.MaxNLocator(integer=True, nbins=12))
    lines_a, labels_a = ax4.get_legend_handles_labels()
    lines_b, labels_b = ax4b.get_legend_handles_labels()
    ax4.legend(lines_a+lines_b, labels_a+labels_b, fontsize=8)

    plt.tight_layout(rect=[0, 0, 1, 0.97])
    if guardar_png is None:
        guardar_png = f"convergencia_aloe_{objetivo}.png"
    try:
        fig.savefig(guardar_png, dpi=150, bbox_inches="tight")
        log.info("Gráfico ALOE guardado: %s", guardar_png)
    except Exception as e:
...
class ResultadoSPSA:
    ...
    max_iter:          int   = 200
    historia_costos:   list  = field(default_factory=list)
    # {iter, costo, f_incumbente, mejor_hasta_ahora, ak, ck, grad_norm, t_seg}
...
def optimizar_spsa(
    ...
    n_reps:    int   = 30,
    A:         float = 10.0,
    guardar_json: str = "resultado_m13_spsa.json",
) -> ResultadoSPSA:
    """SPSA: gradiente por perturbación simultánea — solo 2 eval/iter."""
    log.info("="*65)
    log.info("SPSA — Módulo 13  |  %s", objetivo)
    log.info("α=%.3f  γ=%.3f  step=%.3f  gavg=%d  n_reps=%d  A=%.1f  max_iter=%d",
             alpha, gamma_exp, step, gavg, n_reps, A, max_iter)
    log.info("="*65)
    ...
    historia.append({"iter": 0, "costo": round(fk, 4),
                     "f_incumbente": round(fk, 4), "mejor_hasta_ahora": round(fk, 4),
                     "ak": 0.0, "ck": 0.0, "grad_norm": 0.0, "t_seg": 0.0})
    log.info("x0: %s=%.4f", objetivo, fk)

    grad_buffer = []   # buffer para promediar gavg gradientes

    for k in range(1, max_iter + 1):
        # Secuencias de ganancia (Spall 1998, eq. 3.4)
        ak = alpha  / (k + A) ** alpha
        ck = step   / (k ** gamma_exp)

        # Perturbación Bernoulli ∈ {-1, +1}^d
        delta_k = rng.choice([-1.0, 1.0], size=_ASTRO_D)
        ...
        # Gradiente SPSA
        gk = (fp - fm) / (2.0 * ck * delta_k)
        grad_buffer.append(gk)
        if len(grad_buffer) > gavg:
            grad_buffer.pop(0)

        g_avg    = np.mean(grad_buffer, axis=0)
        ...
        if f_new < f_inc:
            f_inc, x_inc = f_new, xk.copy()

        t_seg = round(time.time() - t0, 2)
        historia.append({
            "iter": k, "costo": round(f_new, 4),
            "f_incumbente": round(f_inc, 4),
            "mejor_hasta_ahora": round(f_inc, 4),
            "ak": round(float(ak), 6), "ck": round(float(ck), 6),
            "grad_norm": round(grad_norm, 6), "t_seg": t_seg,
        })
        log.info("k=%3d  %s=%.3f  inc=%.3f  ak=%.5f  ||g||=%.4f",
                 k, objetivo, f_new, f_inc, ak, grad_norm)

        if ak < 1e-8:
            log.info("Convergencia: ak < 1e-8")
            break

    t_total  = time.time() - t0
    inc_dict = _norm_to_cfg(x_inc)
    log.info("─"*65)
    log.info("SPSA completado: %.0fs | Mejor %s = %.4f", t_total, objetivo, f_inc)

    resultado = ResultadoSPSA(
        objetivo=objetivo, n_iter=len(historia), incumbente=inc_dict,
        costo_incumbente=round(f_inc, 4), tiempo_seg=round(t_total, 2),
        seed=seed, seed_base=seed_base, alpha=alpha, gamma=gamma_exp,
        step=step, gavg=gavg, n_reps=n_reps, A=A, max_iter=max_iter,
        historia_costos=historia,
    )
    out = dataclasses.asdict(resultado)
    with open(guardar_json, "w", encoding="utf-8") as f:
        json.dump(out, f, ensure_ascii=False, indent=2)
    log.info("Guardado: %s", guardar_json)
    if not getattr(optimizar_spsa, "_no_plot", False):
        graficar_spsa(resultado, guardar_png=guardar_json.replace(".json", ".png"))
    return resultado


def graficar_spsa(resultado, guardar_png=None, mostrar=False):
    """4 paneles: convergencia + tiempo + paso ak + ||gk|| norma gradiente."""
    r = resultado.__dict__ if hasattr(resultado, "__dict__") else dict(resultado)
    historia  = r.get("historia_costos", [])
    objetivo  = r.get("objetivo", "tts_full_days_mean")
    t_total   = float(r.get("tiempo_seg", 0.0))
    ...
    inc_cum   = [h.get("mejor_hasta_ahora", float("nan")) for h in historia]
    t_secs    = [h.get("t_seg", 0.0)      for h in historia]
    aks       = [h.get("ak", 0.0)         for h in historia]
    gnorms    = [h.get("grad_norm", 0.0)  for h in historia]
    t_delta   = [t_secs[0]] + [max(0.0, t_secs[i]-t_secs[i-1]) for i in range(1, len(t_secs))]
    ...
    fig, axes = plt.subplots(4, 1, figsize=(10, 14),
                              gridspec_kw={"height_ratios": [3, 1.5, 1.5, 1.5]})
    ax1, ax2, ax3, ax4 = axes
    fig.suptitle(f"SPSA — Perturbación Simultánea\nObjetivo: {objetivo}  ·  "
                 f"{n_iter} iteraciones  ·  {t_total:.0f}s totales",
                 fontsize=11, fontweight="bold", y=0.99)

    # Panel 1: Convergencia
    ax1.scatter(iters, costos, s=18, c="#aaaaaa", alpha=0.5, zorder=2, label="Evaluaciones")
    inc_v = [(i, v) for i, v in zip(iters, inc_cum) if not np.isnan(v)]
    if inc_v:
        xi, yi = zip(*inc_v)
        ax1.step(xi, yi, where="post", color="#1a6db5", linewidth=2.2, zorder=3, label="Incumbente")
    ...
    if not np.isinf(costo_fin):
        ax1.axhline(costo_fin, color="#c0392b", linewidth=1.2, linestyle="--",
                    alpha=0.75, label=f"Mejor={costo_fin:.2f}")
    ax1.set_xlabel("Iteración", fontsize=10); ax1.set_ylabel(objetivo, fontsize=10)
    ax1.set_title("Convergencia SPSA", fontsize=10, pad=4)
    ax1.legend(fontsize=8); ax1.xaxis.set_major_locator(mticker.MaxNLocator(integer=True, nbins=12))
    ...
    # Panel 3: Paso ak (secuencia decreciente)
    aks_pos = [max(a, 1e-12) for a in aks]
    ax3.semilogy(iters, aks_pos, color="#8e44ad", linewidth=1.8, label="ak = α/(k+A)^α")
    ax3.axhline(1e-8, color="#c0392b", linewidth=0.8, linestyle="--", alpha=0.6, label="conv tol")
    ax3.set_title("Secuencia de paso ak (log)", fontsize=10, pad=4)
    ax3.set_xlabel("Iteración", fontsize=10); ax3.set_ylabel("ak", fontsize=9)
    ax3.xaxis.set_major_locator(mticker.MaxNLocator(integer=True, nbins=12))
    ax3.legend(fontsize=8)

    # Panel 4: Norma del gradiente SPSA
    ax4.plot(iters, gnorms, color="#e67e22", linewidth=1.5, alpha=0.85, label="‖ĝk‖")
    ax4.fill_between(iters, gnorms, alpha=0.15, color="#e67e22")
    if gnorms:
        gmed = float(np.median(gnorms))
        ax4.axhline(gmed, color="#333333", linewidth=0.9, linestyle=":", label=f"mediana={gmed:.4f}")
    ax4.set_title("Norma del gradiente SPSA ‖ĝk‖", fontsize=10, pad=4)
    ax4.set_xlabel("Iteración", fontsize=10); ax4.set_ylabel("‖ĝk‖", fontsize=9)
    ax4.xaxis.set_major_locator(mticker.MaxNLocator(integer=True, nbins=12))
    ax4.legend(fontsize=8)

    plt.tight_layout(rect=[0, 0, 1, 0.97])
    if guardar_png is None:
        guardar_png = f"convergencia_spsa_{objetivo}.png"
    try:
        fig.savefig(guardar_png, dpi=150, bbox_inches="tight")
        log.info("Gráfico SPSA guardado: %s", guardar_png)
    except Exception as e:
        log.warning("Error guardando %s: %s", guardar_png, e)
    if mostrar:
        plt.show()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Módulo 14: ALOE — Adaptive Line-search with Oracle Estimations"
    )
    ...
    parser.add_argument("--guardar_json", type=str,   default="resultado_m14_aloe.json")
    ...
    parser.add_argument("--gamma",     type=float, default=0.8,
                        help="Factor reducción paso si Armijo falla (default: 0.8)")
    parser.add_argument("--alpha_max", type=float, default=10.0,
                        help="Paso máximo (default: 10.0)")
    parser.add_argument("--alpha_0",   type=float, default=1.0,
                        help="Paso inicial (default: 1.0)")
    parser.add_argument("--epsilon_f", type=float, default=1.0,
                        help="Constante aditiva Armijo (absorbe ruido, default: 1.0)")
    parser.add_argument("--h",         type=float, default=0.01,
                        help="Paso diferencias finitas para gradiente (default: 0.01)")
    args = parser.parse_args()
    optimizar_aloe._no_plot = args.no_plot
    ...
    parser.add_argument("--seed_base",    type=int,   default=202)
    parser.add_argument("--n_workers",    type=int,   default=0)
    parser.add_argument("--guardar_json", type=str,   default="resultado_m13_spsa.json")
    parser.add_argument("--no_plot",      action="store_true")
    # Parámetros SPSA (Spall 1998)
    parser.add_argument("--alpha",     type=float, default=0.602,
                        help="Exponente secuencia ak (default: 0.602)")
    parser.add_argument("--gamma_exp", type=float, default=0.101,
                        help="Exponente secuencia ck (default: 0.101)")
    parser.add_argument("--step",      type=float, default=0.1,
                        help="Magnitud inicial perturbación ck (default: 0.1)")
    parser.add_argument("--gavg",      type=int,   default=1,
                        help="N° gradientes a promediar (default: 1)")
    ...
    parser.add_argument("--A",         type=float, default=10.0,
                        help="Estabilizador secuencia ak (default: 10.0)")
    args = parser.parse_args()
    optimizar_spsa._no_plot = args.no_plot
    ...
    optimizar_spsa(
        x0=x0, seed=args.seed, seed_base=args.seed_base,
        objetivo="tts_full_days_mean", n_workers=args.n_workers,
        max_iter=args.n_iter, alpha=args.alpha, gamma_exp=args.gamma_exp,
        step=args.step, gavg=args.gavg, n_reps=args.n_reps, A=args.A,
        guardar_json=args.guardar_json,
    )
    parser.add_argument("--incumbente",   type=str, default=None,
