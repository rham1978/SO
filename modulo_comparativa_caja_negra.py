...
def _convergencia_smac(historia_costos: list, n_corridas_eval: int = 2):
    """
    Para módulos SMAC (M4-M10): extrae (eval_acumulada, mejor_hasta_ahora).
    Ordena por config_id para asegurar orden cronológico.
    Cada entrada = 1 trial ≈ n_corridas_eval evaluaciones.
    """
    # Ordenar por config_id (orden de evaluación en SMAC)
    entries = sorted(historia_costos, key=lambda e: e.get("config_id", 0))
    mejores = []
    mejor   = float("inf")
    n_eval  = 0
    for e in entries:
        costo = e.get("costo", float("inf"))
        n_eval += n_corridas_eval
        if costo < mejor:
            mejor = costo
...
def run_m14_aloe(max_iter: int = 50, r: int = 5, n_workers: int = 0, seed: int = 42) -> dict:
    ...
    # ALOE: por iteración → gradiente (2*d evals) + 2 evals para Armijo
    _d = 12
    evals_grad = 2 * _d * max(2, r // 4)
    evals_arm  = 2 * r
...
def _tiempos_smac(historia_costos: list, tiempo_total: float) -> list:
    """
    Interpola tiempos linealmente para módulos SMAC (no guardan t_seg por trial).
    Ordena por config_id para orden cronológico.
    """
    entries  = sorted(historia_costos, key=lambda e: e.get("config_id", 0))
    mejores  = []
    mejor    = float("inf")
    n        = len(entries)
    dt_trial = tiempo_total / max(n, 1)
