"""
===============================================================================
MÓDULO 8 — SK con Replicación Adaptativa
===============================================================================
Extiende el Módulo 7 (SMAC + Stochastic Kriging) con replicación adaptativa:
en vez de usar n_corridas fijo por evaluación, el número de réplicas n(x)
se decide dinámicamente en función de la varianza σ²(x) estimada por el SK.

Principio:
  · Zonas del espacio donde el simulador es ruidoso (σ² alto) reciben
    más réplicas → mejor estimación de la media real.
  · Zonas donde el simulador es estable (σ² bajo) reciben menos réplicas
    → ahorro de tiempo de cómputo.
  · El SK ya mide σ²(x) en cada evaluación → se reutiliza sin costo extra.

Regla de asignación:
  n(x) = n_min + round( rank_percentil(σ²(x)) × (n_max - n_min) )
  donde rank_percentil es la posición relativa de σ²(x) entre todas las
  varianzas conocidas hasta ese momento.

Fases:
  1. Warmup (primeras n_warmup configs): usa n_min réplicas para
     poblar _variance_store antes de adaptar.
  2. Adaptativa (resto): n(x) según varianza relativa estimada.

Parámetros nuevos vs módulo 7:
  --n_min_reps   : réplicas mínimas (default 2, configs poco ruidosas)
  --n_max_reps   : réplicas máximas (default 8, configs muy ruidosas)
  --n_warmup     : configs de warmup con n_min (default 10)

Indicador fijo: tts_full_days_mean
Gráficos: convergencia + tiempo + réplicas adaptativas + σ² por evaluación

Uso:
  python modulo8_sk_adaptativo.py --tipo hpo_sk --n_trials 100
  python modulo8_sk_adaptativo.py --tipo hpo_sk --n_trials 100 --n_workers 4
  python modulo8_sk_adaptativo.py --tipo hpo_sk --n_trials 100 \\
      --n_min_reps 2 --n_max_reps 8 --n_warmup 10
  python modulo8_sk_adaptativo.py --evaluar_incumbente resultado_m8.json
===============================================================================
"""
...
class ResultadoSMAC_SK:
    ...
    sigma_history:    list  = field(default_factory=list)  # σ² por evaluación
...
def graficar(
    ...
    # Número de filas: 2 base + 2 extra si adaptativo
    n_panels = 4 if (adaptativo and n_reps_history and sigma_history) else 2
    height_ratios = [3, 2, 1.5, 1.5] if n_panels == 4 else [3, 2]
    ...
    modo = "Adaptativo" if adaptativo else "Fijo"
    reps_str = (f"n_reps [{n_min_reps}–{n_max_reps}] adaptativo"
                if adaptativo else f"{n_corridas} réplicas/config")
    fig.suptitle(
        f"SMAC+SK — {nombre_tipo}  [{modo}]\n"
    ...
    # ── Paneles 3 y 4: solo si adaptativo ────────────────────
    if n_panels == 4:
        ...
        # Panel 4: σ² (varianza simulador) por evaluación
        sigma_n = sigma_history[:n]
        sigma_trials = list(range(1, len(sigma_n) + 1))
        if sigma_n:
            ax4.fill_between(sigma_trials, sigma_n, alpha=0.25, color="#c0392b")
            ax4.plot(sigma_trials, sigma_n, color="#c0392b", linewidth=1.5,
                     label="σ² simulador")
            sigma_med = float(np.median(sigma_n))
            ax4.axhline(sigma_med, color="#333333", linewidth=1.0,
                        linestyle="--", alpha=0.6, label=f"mediana={sigma_med:.1f}")
            if n_warmup > 0:
                ax4.axvline(n_warmup + 0.5, color="#8e44ad", linewidth=0.9,
                            linestyle="--", alpha=0.6)
        ax4.set_xlabel("N° evaluación", fontsize=10)
        ax4.set_ylabel("σ² (varianza)", fontsize=9)
        ax4.set_title("Varianza simulador por evaluación", fontsize=10, pad=4)
    ...
    if guardar_png is None:
        modo_sfx = "adaptativo" if adaptativo else tipo
        guardar_png = f"convergencia_{modo_sfx}_{objetivo}.png"
    try:
...
def optimizar(
    ...
    n_workers:       int   = 0,
    # ── Parámetros replicación adaptativa (módulo 8) ──────────
    ...
    """
    Ejecuta SMAC+SK con replicación adaptativa (módulo 8).

    adaptativo=True  → n_reps decidido por AdaptiveReplicationPolicy
    adaptativo=False → n_corridas_eval fijo (compatible con módulo 7)
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Módulo 8: SK con Replicación Adaptativa — DES heteroscedástico"
    )
    ...
    parser.add_argument("--n_corridas",  type=int,   default=3,
                        help="Réplicas base (usado si --no_adaptativo, default: 3)")
    ...
    parser.add_argument("--no_adaptativo", action="store_true",
                        help="Desactiva replicación adaptativa (usa n_corridas fijo)")
    parser.add_argument("--n_min_reps",  type=int,   default=2,
                        help="Réplicas mínimas en modo adaptativo (default: 2)")
    parser.add_argument("--n_max_reps",  type=int,   default=8,
                        help="Réplicas máximas en modo adaptativo (default: 8)")
    parser.add_argument("--n_warmup",    type=int,   default=10,
                        help="Evaluaciones de warmup con n_min antes de adaptar (default: 10)")
    # ── Otros ───────────────────────────────────────────────
    ...
    if args.evaluar_incumbente:
        ...
        evaluar_incumbente(
            ...
            guardar_json = "evaluacion_incumbente_m8.json",
    ...
    optimizar(
        ...
        n_workers        = args.n_workers,
        adaptativo       = not args.no_adaptativo,
